...
export const validationInputs = (
    ...
    const addDangerOrderByCondition = () => {
        inputs.forEach(input => {
            input.value.length ? input.classList.remove("border-danger") : input.classList.add("border-danger")
        })
    }

    if (!textInputValue || !priceInputValue || !dateInputValue) {
        handleAlertMessage({ alertText: "Заполните все поля", alertStatus: "warning" })
        addDangerOrderByCondition()
        return false
    }
    ...
    inputs.forEach(el => el.classList.remove("border-danger"))
